<?php
    ...
    function add_new_attachment($file, $productName, $artDir, $postId, $token){
      ...
      rentman_import_log("     Rentman url = " . $file[0] . "\n", 0);
      rentman_import_log("     File size = " . number_format($filesize / 1024 / 1024, 2) . " MB\n", 0);
      ...
      $targetUrl = WP_CONTENT_DIR . $artDir . $guid;
      rentman_import_log("     WordPress url = " . $targetUrl . "\n", 0);

      if (!file_exists($targetUrl)) {
          if(!@copy($fileUrl,$targetUrl)){
              $errors= error_get_last();
              rentman_import_log("     " . $errors['message'] . "\n", 0);
              if(strpos($errors['message'], "failed to open stream") !== false && strpos($errors['message'], "amazon")){
                  rentman_import_log("     Error = Rentman image doesn't exist on amazon\n", 0);
              }
          }
      }
