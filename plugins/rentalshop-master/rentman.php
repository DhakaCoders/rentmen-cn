<?php
    ...
    function menu_display(){
        ...
        if (isset($_GET['import_products'])){
            ...
            array_to_product($prod_array, $file_array, $pdf_array, (int)$array_index,$token, (int)$upload_batch);
        }
    ...
    function login_user($showerrormsgs){
        ...
        if (completedata() == "complete"){
            ...
            if ($parsed['response']['login'] == false){
                ...
                # Check if the authentication has permissions for exporting projects to rentman
                $message = json_encode(check_permissions($token), JSON_PRETTY_PRINT);
                $received = do_request($url, $message);
                $parsed = json_decode($received, true);
                $haspermissions = $parsed['response']['value']['valid'];
                if($showerrormsgs) {
                    if($haspermissions){
                        echo("<br>");
                      ...
                      echo("<br>");
                      //_e('&#x274c; This account does not have the right permissions.', 'rentalshop');
                      //echo('<br>&#8505; <font style="color:#FF0000;">' . $parsed['response']['value']['reason'] . '</font>');
                      echo('&#x274c; ' . $parsed['response']['value']['reason']);
                      return "fail";
                    }
                }
                if(!$haspermissions){
    ...
    function check_compatibility()
    {
        ...
        $fileUrl = 'https://raw.githubusercontent.com/rentmanpublic/rentalshop/master/img/test.png';
        $fileUrlHtaccess = 'https://raw.githubusercontent.com/rentmanpublic/rentalshop/master/img/.htaccess';
