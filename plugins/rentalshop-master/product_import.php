<?php
        ...
        if ($token == "fail"){
            ...
            if (apply_filters('rentman/import_products', true)) {
                ...
                if ($listLength > 0) { # At least one product has been found
                    ...
                    $images_array = array();
                    $import_log = "Import: " . date("d-m-Y h:i:s a") . "\nTotal products: " . $nbrOfProducts . "\n\nPRODUCTS TO IMPORT\n==================\n\n";
                    for ($z = 0; $z < $nbrOfProducts; $z++) {
                        $import_log.= ($z + 1) . ": " . $prod_array[$z]["name"]  . "\n";
                        $key = $prod_array[$z]["id"];
                    ...
                    if (sizeof($prod_array) > 0) {
                        ...
                        # Prepare Script
                        wp_register_script('admin_add_product', plugins_url('js/admin_import.js?' . get_plugin_data(realpath(dirname(__FILE__)) . '/rentman.php')['Version'], __FILE__));
                        ...
                        wp_localize_script('admin_add_product', 'uploadbatch', $uploadbatch);
        ...
        if (sizeof($prod_array) > 0){
            ...
            for ($index = $startIndex; $index <= $endIndex; $index++){
                ...
                if($endIndex > $total_products){
                    ...
                    rentman_import_log(($index + 1) . " / " . ($endIndex + 1) . " - ", 0);
                }
    ...
    function convert_items($parsed, $lower, $higher){
        ...
        for ($x = $lower; $x <= $higher; $x++) {
            ...
            $product_id = $wpdb->get_var($wpdb->prepare("SELECT posts.ID FROM $wpdb->posts AS posts LEFT JOIN $wpdb->postmeta AS postmeta ON ( posts.ID = postmeta.post_id ) WHERE posts.post_type IN ( 'product', 'product_variation' ) AND postmeta.meta_key = '_sku' AND postmeta.meta_value = '%s' AND posts.post_status = 'publish' LIMIT 1", $x));
    ...
    function reset_rentman(){
        $posts = get_product_list_rentman();
